...
@dataclass
class ProjectIPResult:
    """IP access analysis result for one project."""
    ip_list: list[str]
    has_open_internet: bool
    error: str | None = None
...
def has_incomplete_results(results: dict[str, ProjectIPResult]) -> bool:
    """Return True when any project could not be checked."""
    return any(result.error for result in results.values())


def print_security_summary(results: dict[str, ProjectIPResult]):
    """
    Print a final security summary report of projects with open internet access.

    Args:
        results: Dictionary mapping project names to ProjectIPResult
    """
    # Collect projects with open internet access
    open_projects = []
    error_projects = []
    for project_name, result in sorted(results.items()):
        if result.error:
            error_projects.append((project_name, result.error))
        if result.has_open_internet:
            # Find which specific entries are open
            open_entries = [ip for ip in result.ip_list if ip in ["0.0.0.0/0", "0.0.0.0"]]
            open_projects.append((project_name, open_entries))

    if not open_projects and not error_projects:
        ...
        print(f"{Colors.GREEN}All checked projects have explicit IP access restrictions configured.{Colors.RESET}\n")
    else:
        print(f"{Colors.BOLD}{'='*80}")
        if open_projects:
            print(f"⚠️  SECURITY SUMMARY: OPEN INTERNET ACCESS DETECTED")
        else:
            print(f"⚠️  SECURITY SUMMARY: CHECKS INCOMPLETE")
        print(f"{'='*80}{Colors.RESET}\n")

        if error_projects:
            print(f"{Colors.YELLOW}{Colors.BOLD}WARNING: The following {len(error_projects)} project(s) could not be checked:{Colors.RESET}\n")
            for idx, (project_name, error) in enumerate(error_projects, 1):
                print(f"{Colors.YELLOW}{idx}. {Colors.BOLD}{project_name}{Colors.RESET}")
        ...
        print(f"  • Re-run any incomplete checks and treat them as unknown until verified")
        print(f"  • Use VPN or bastion hosts for secure database access")
        print(f"  • Regularly audit IP access lists for compliance\n")


def print_results(results: dict[str, ProjectIPResult]):
    """
    ...
    open_internet_projects = sum(1 for result in results.values() if result.has_open_internet)
    incomplete_projects = sum(1 for result in results.values() if result.error)
    ...
    print(f"  Projects not checked: {Colors.YELLOW}{incomplete_projects}{Colors.RESET}\n")

    for project_name, result in sorted(results.items()):
        if result.error:
            status_icon = f"{Colors.YELLOW}⚠️  UNKNOWN{Colors.RESET}"
        elif result.has_open_internet:
            status_icon = f"{Colors.RED}⚠️  OPEN{Colors.RESET}"
        else:
        ...
        if result.error:
            print(f"  {Colors.YELLOW}Unable to check: {result.error}{Colors.RESET}")
        elif not result.ip_list:
            print(f"  {Colors.YELLOW}(empty - no access list entries configured){Colors.RESET}")
        else:
...
def main():
    ...
    if not all([org_id, public_key, private_key]):
        ...
        print("\nUsage:")
        print("  python3 atlas_ip_access_analyzer.py")
        print("  python3 atlas_ip_access_analyzer.py <ORG_ID>")
    ...
    try:
        ...
        for project in projects:
            ...
            try:
                ...
                results[project_name] = ProjectIPResult(ips, has_open)
                print("✓")
            except Exception as e:
                print(f"✗ Error: {str(e)}")
                results[project_name] = ProjectIPResult([], False, str(e))
        ...
        for project_name, result in results.items():
            json_output["projects"][project_name] = {
                "ip_access_list": result.ip_list,
                "has_0_0_0_0": result.has_open_internet,
                "entry_count": len(result.ip_list),
                "error": result.error,
            }
        ...
        sys.exit(1 if has_incomplete_results(results) else 0)
